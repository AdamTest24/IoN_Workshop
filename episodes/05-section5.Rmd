---
title: "Neuro Images Clustering"
teaching: 10
exercises: 2
---

[**Download Chapter notebook (ipynb)**](05-section5.ipynb)

:::::::::::::::::::::::::::::::::::::: questions 

- What is NiBabel?
- How is data processed in NiBabel?
- How is machine learning applied on image data?

::::::::::::::::::::::::::::::::::::::::::::::::

::::::::::::::::::::::::::::::::::::: objectives

- Installing and using NiBabel Python Library
- Understanding data pre-processing
- Segmenting images with Gaussian Mixtures

::::::::::::::::::::::::::::::::::::::::::::::::


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(reticulate)
use_python("/Users/sabaferdous/envL2D/bin/python3")
#use_virtualenv("r-env")
```

## NiBabel Images
```{python}
import nibabel as nib
```

:::::::::::::::::::::: callout
## Note
Note how we import the NiBabel package as `nib`. You can use any abbreviation to access the package’s functions from within your programme.

To familiarise yourself with the NiBabel package, try the [Getting started tutorial](https://nipy.org/nibabel/gettingstarted.html) using an example image file.

::::::::::::::::::::::

### **Load images and get data**

NiBabel has its own import function `load()`.

```{python}
img_nibabel = nib.load("data/T1_mask.nii")

type(img_nibabel)
```

The header lets you see metadate that come with the image.

```{python}
meta_info = img_nibabel.header

print(meta_info)
```

```{python}
print(meta_info.get_xyzt_units())
```

With `get_fdata()` the image intensities can be converted to a Numpy array.

```{python}
img1 = img_nibabel.get_fdata()

img1.shape
```

```{python}
img_nibabel = nib.load("data/b0_mask.nii")

img2 = img_nibabel.get_fdata()
```

We now import a second image with same spatial resolution but additional information from another imaging modality.

```{python}
img_nibabel = nib.load("data/b0_mask.nii")

img2 = img_nibabel.get_fdata()

img2.shape
```

For simpler handling, we only select a two-dimensional slice in each 3D image.

### **Plot Images**

```{python}
img_slice = 30

img1_slice = img1[:, :, img_slice]
img2_slice = img2[:, :, img_slice]
```

```{python}
from matplotlib.pyplot import subplots, show

fig, ax = subplots(ncols=2, figsize=(15, 5))

f1 = ax[0].imshow(img1_slice, cmap="gray")
f2 = ax[1].imshow(img2_slice, cmap="gray")

fig.colorbar(f1, ax=ax[0]);
fig.colorbar(f2, ax=ax[1]);

ax[0].set_xlabel('T1 Image', fontsize=16);
ax[1].set_xlabel('B0 Image', fontsize=16);

show()
```

To check how similar the images are, we can compare their intensity histograms.

### **Histograms**
```{python}
mask = (img1_slice>0) & (img2_slice>0) 

img1_nz = img1_slice[mask]
img2_nz = img2_slice[mask]

fig, ax = subplots(nrows=1, ncols=2, figsize=(20, 5))

ax[0].hist(img1_nz, bins=50);
ax[1].hist(img2_nz, bins=50);

show()
```

These two images appear to have different, potentially complementary information.

### **Visualise and Concatenate**

For the Seaborn library, sees: https://seaborn.pydata.org

Here is the documentation for the kernel density plot:
https://seaborn.pydata.org/generated/seaborn.kdeplot.html

We now can compare the scatter plot, a two-dimensional hisstogram, and a kernel density contour plot.

```{python}
fig, ax = subplots(1, 3, figsize=(20, 6))

# Scatter plot
ax[0].scatter(img1_nz, img2_nz)

# 2D Histogram
ax[1].hist2d(img1_nz, img2_nz, bins=50, vmax=10);

from seaborn import kdeplot

# Density Plot
kdeplot(x=img1_nz, y=img2_nz, ax=ax[2]);

show()
```

For Machine Learning, the data need to be in a single two-dimensional array. Rows denote poixels and there are two columns for two images.

```{python}
from numpy import concatenate

all_imgs = concatenate([img1_nz.reshape(-1,1), img2_nz.reshape(-1,1)], axis=1)

all_imgs.shape
```


## Gaussian Mixture Clustering
After the data preparation steps, we can proceed with our machine learning analysis. We want to segment the images into different types of tissues. It is not obvious how to do this, as the intensity values in the above histograms are distributed without obvious segementation of intensities. We will nonetheless attempt to cluster the images using a Gaussian Mixture Model (GMM).

The result of the clustering are labels for each of the pixels, according to the tissue type which the algorithms assignss

Here is the workflow:

1. We import the GMM class from the Python machine learning library __Scikit-learn__.

2. We decide the number of clusters that we expect to see.

3. We set a random seed to make the outcome reproducible.

4. We instantiate the clustering algorithm.

5. We use function `fit_predict` to fit the model to the data and obtain the labels for each pixel. 

```{python}
from sklearn.mixture import GaussianMixture
```

```{python}
n_components = 3

RANDOM_STATE = 12345

gmm = GaussianMixture(n_components=n_components, 
                      random_state=RANDOM_STATE)

all_img_labels = gmm.fit_predict(all_imgs)

all_img_labels[0]
```

We can plot a scatter plot of the labels to see their distribution.

```{python}
fig, ax = subplots(figsize=(4, 4))

ax.scatter(img1_nz, img2_nz, c=all_img_labels, s=100)

ax.set_xlabel('Image 1', fontsize=16)
ax.set_ylabel('Image 2', fontsize=16);

show()
```

In the final step, we need to map the predicted labels from the "intensity space" back to the original image in its anatomical space.

```{python}
from numpy import zeros

all_img_labels_mapped = zeros(img1_slice.shape)

all_img_labels_mapped[mask] = all_img_labels
```

```{python}
fig, ax = subplots(figsize=(4, 4))

ax.imshow(all_img_labels_mapped);

show()
```

:::::::::::::::::::::::::::: challenge
## Exercise
Redo the above sequence of steps with different pre-assigned number of clusters and compare the results.

::::::::::::::::::::::::::::

:::::::::::::::::::::::::::::::::::::keypoints 

- `NiBabel` supports an ever growing collection of neuroimaging file formats.
- Images can be clustered using a `Gaussian Mixture Model (GMM)`.
- `fit_predict` function is used to fit the model to the data and obtain the labels for each pixel. 

::::::::::::::::::::::::::::::::::::::::::::::::
[r-markdown]: https://rmarkdown.rstudio.com/
